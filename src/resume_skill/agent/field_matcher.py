...
def match_fields_with_llm(
    ...
    # Run LLM matching only for fields that need it
    llm_matches: dict[int, dict[str, Any]] = {}
    if needs_llm:
        fields_summary = _prepare_fields_for_prompt(needs_llm)
...
每个字段都必须出现在matches或no_matches中。"""
        ...
        try:
            ...
            print(f"[Matcher] LLM matching failed: {e}, using rule-based fallback")
