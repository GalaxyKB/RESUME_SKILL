...
class PersonalInfoExtractor:
    ...
    def extract_from_resume_pdf(self, resume_path: str, output_to_template: bool = True) -> dict[str, Any]:
        ...
        extraction_prompt = f"""你是专业简历解析AI。请从以下简历中提取**全部**信息，尽可能详细，不要遗漏任何内容。
...
1. 提取简历中出现的**所有**信息，上面清单之外的内容也要提取出来（证书编号、竞赛名次、兴趣爱好等）
2. 如果简历中有出生日期，自动计算**年龄**并提取（用当前时间2026年计算）
3. 如果简历中有多项教育/工作/项目经验，全部列出，不要只取第一条
4. 如果简历中有实习起始时间，自动计算**工作年限/实习时长**
5. 简历中任何数字、日期、名称、链接、等级都不要遗漏

## 需要提取的字段（请逐项提取，有则填值，无则跳过）

**个人基本信息：**
- 中文姓名、英文/拼音姓名、性别、出生日期、**年龄**、邮箱、手机号、微信号、现居住地、期望工作城市、户口所在地、民族、政治面貌、婚姻状况、到岗时间、求职状态、身份证号、籍贯、个人主页/博客/GitHub

**教育背景（每段都提取）：**
- 学校名称、学院、学位/学历、专业、入学时间、毕业时间、GPA、排名、主修课程

**工作/实习经历（每段都提取）：**
- 公司名称、职位、开始时间、结束时间、职责与成就、汇报对象

**项目经验（每段都提取）：**
- 项目名称、角色、开始时间、结束时间、技术栈、项目描述、成就

**技能：**
- 编程语言（列出全部）、框架、工具、领域/方向、语言能力及等级

**获奖/证书（每项都提取）：**
- 名称、时间、等级/名次

**其他：**
- 论文/专利、竞赛经历、兴趣爱好、自我评价
